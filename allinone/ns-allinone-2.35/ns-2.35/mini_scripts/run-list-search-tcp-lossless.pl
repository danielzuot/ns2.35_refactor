...
`ssh $user\@$ns_source_server 'cd $ns_source_path && tar cvzf ns-latest.tar.gz ns-allinone-2.35 >> /dev/null'`;
...
`ssh $user\@$log_server 'mv $log_dir/logs $log_dir/$mon$mday-search-tcp-lossless-mini'`;
